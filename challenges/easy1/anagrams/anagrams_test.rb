...
class AnagramTest < Minitest::Test
  ...
  def test_anagrams_contain_non_letters
    # skip
    detector = Anagram.new('Orchestra')
    assert_raises(NonLetterError) do
      anagrams = detector.match %w(cashregister Carthorse! radishes)
    end
  end

  def test_base_word_contain_non_letters
    # skip
    detector = Anagram.new('william!')
    assert_raises(NonLetterError) do
      detector.match %w(aimwill!)
    end
  end
end
