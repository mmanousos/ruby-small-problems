...
# `#process` method or part of the blocks. You need no other additions or
# changes.
#
...
class TextAnalyzer
  def process
    text = File.open('sandwich_code.txt')
...
analyzer = TextAnalyzer.new
analyzer.process { |doc| puts "#{doc.scan(/\n{2}|\z/).size} paragraphs" }
analyzer.process { |doc| puts "#{doc.lines.size} lines" }
analyzer.process { |doc| puts "#{doc.scan(/\b[a-z]+?\b/i).size} words" }

# analyzer.process { |doc| puts "#{doc.split("\n\n").count} paragraphs" }
# analyzer.process { |doc| puts "#{doc.split("\n").count} lines" }
# analyzer.process { |doc| puts "#{doc.split(' ').count} words" }
