...
works (https://launchschool.com/lessons/c0400a9c/assignments/26d715d8).
To review briefly, consider this code:
...
block. If that object is a proc, the conversion happens automagically, just as
shown above. If the object is not a proc, then & attempts to call the #to_proc
method on the object first. Used with symbols, e.g., `&:to_s`, Ruby creates a
proc that calls the `#to_s` method on a passed object, and then converts that
proc to a block. This is the "symbol to proc" operation (though perhaps it
should be called "symbol to block").
...
base 8. Use the comments for help in determining where to make your
