...
# shown above. If the object is not a proc, then `&` attempts to call the
# `#to_proc` method on the object first. Used with symbols,
...
def convert_to_base_8(n)
  n.to_s(8).to_i # convert the number to a string with base 8, convert back to integer
end

# The correct type of argument must be used below
base8_proc = method(:convert_to_base_8).to_proc # make the method a proc by first converting to a symbol

# We'll need a Proc object to make this code work. Replace `a_proc`
# with the correct object
p [8, 10, 12, 14, 16, 33].map(&base8_proc) # pass the proc as an argument
