...
modify #bubble_sort! so it takes an optional block that determines which of two
consecutive elements will appear first in the results.
=end

# using #each_cons.with_index without block
# def bubble_sort!(array)
#
#   loop do
#     swapped = false
#     array.each_cons(2).with_index do |pair, index|
...
# based on Jerome's solution in the student submissions, you yield one element,
# then compare it to the yield of a second element. (only line 140 changes)
...
def bubble_sort!(array)
  ...
  loop do
    ...
    array.each_cons(2).with_index do |pair, index|
      if block_given?
        next if yield(pair[0]) <= yield(pair[1])
      else
        next if pair[0] <= pair[1]
      end
