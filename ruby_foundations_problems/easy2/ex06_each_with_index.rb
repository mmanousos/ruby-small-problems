...
Algorithm
  BEGIN
    SET index = 0
    WHILE elements remain to evaluate in the argument array
      yield array[index] and index to block
      increment index += 1
    END
    return reference to argument array
  END
...
# Course Solution - using #each instead of simple loop; more efficient since by
# default, #each returns the calling array
# def each_with_index(array)
#   index = 0
#   array.each do |el|
#     yield(el, index)
