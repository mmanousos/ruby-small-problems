...
*algorithm*
BEGIN
  declare a mutating method that accepts an array (arr) of at least two elements as an argument
    SET swapped = false (a variable to hold whether any swaps were made during iteration)
    loop through arr continuiously
      WHILE counter < arr.size
        check if the element at the current position in the array is greater than the element to its left
        IF element to the left is greater
          swap them
          reassign swapped = true
...
def bubble_sort!(arr)
  loop do
    swapped = false
    # loop through the array checking if each pair of elements are in asscending order.
    # if no, swap them
