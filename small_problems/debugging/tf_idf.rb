...
It was a rounding error in the idf method. Because Ruby automatically rounds
integers, the number of documents and the number of documents containing the
term needed to be converted to floats and then divided by one another before
calculating the log. Otherwise the log of their quotient was continually being
calculated down to 0. This resulted in the final product of the tfidf method
being 0 (because multiplying by 0 = 0).
...
# Course Solution: Numeric#fdiv - float division method which converts the
# argument to a floating point number and then divides the caller by it,
# returning a float
