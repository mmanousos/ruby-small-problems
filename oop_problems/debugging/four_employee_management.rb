...
# By changing the attr_reader `serial_number` and `abbreviated_serial_number`
# methods from `private` to `protected`, we allow the rest of the class to
# access these methods while still not allowing them to be called from outside
# of the class. This allows the program to run as expected.

# `private` methods can not be called with an explicit receiver, but `protected`
# methods can. By changing `attr_reader :serial_number` to protected, the
