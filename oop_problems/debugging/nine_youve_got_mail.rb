...
ellens_email_account = EmailService.new('Ellen', 85, 'ellen@talk.com')
...
# puts rafaels_email_account.send(ellens_email_account.email_address, Email.new('Hi Ellen', "It's Rafael. How are you?"))
...
# Based on the error, we can deduce that we can not use an address for the #send
# method on the `johns_postal_service` object.
# Inspecting the #send method, it appears we need three attributes: mail, name,
# and destination. `name` is defined when the `PostalService` object is
# instantiated.
# Upon further inspection, the `#send` method is within the `Mailing` module
# which had been mixed into the `EmailService` class but not the `PostalService`
# class. Correcting for this omission provides the expected results.
# `send` is also a built in method of the `Object` class so this overrides it.
