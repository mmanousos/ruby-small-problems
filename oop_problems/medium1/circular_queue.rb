...
class CircularQueue
  ...
  def enqueue(value)
    if @array.none?{ |el| el == nil }
      @array.shift
      @array.push(value)
    else
      @array[@array.index(nil)] = value
    end
  end

  def dequeue
    @array.push(nil).shift
  end
end

# Use an array:
# items go into slots sequentially as they are added, starting with 0 index - end (push)
# items are removed from slot 0 as they are dequeued (shift) moving all other items forward one position
# if non-nil values of array == buffer size, remove element from 0 and add new element to -1
